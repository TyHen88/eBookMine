  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-4 py-8 md:pl-24">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-gradient-to-br from-white via-indigo-50/20 to-brand-50/30 p-6 sm:p-8 shadow-xl shadow-brand-500/5 backdrop-blur-xl dark:border-slate-800/80 dark:from-slate-900/90 dark:via-slate-900/60 dark:to-indigo-950/20 mb-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex items-center gap-4 sm:gap-5">
              {user?.image ? (
                ...
                <img
                  src={user.image}
                  alt=""
                  className="h-16 w-16 sm:h-20 sm:w-20 rounded-3xl object-cover ring-4 ring-white dark:ring-slate-800 shadow-xl"
                />
              ) : (
                <div className="flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-3xl bg-gradient-to-tr from-brand-600 via-indigo-600 to-purple-600 text-white font-extrabold text-2xl shadow-xl shadow-brand-500/20">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-full bg-brand-50 px-2.5 py-0.5 text-[10px] font-extrabold text-brand-700 dark:bg-brand-950/60 dark:text-brand-300 border border-brand-200/60">
                    <CrownIcon size={11} className="text-amber-500" />
                    {user?.role || "READER"}
                  </span>
                </div>
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">
                  Member of eBookMine Collaborative Reading
                </p>
              </div>
            </div>
            ...
            <button
              onClick={() => signOut()}
              className="flex items-center gap-1.5 px-4 py-2 rounded-2xl border border-slate-200 bg-white hover:bg-red-50 hover:border-red-200 hover:text-red-600 text-xs font-bold text-slate-600 shadow-sm transition-all dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-red-950/40 dark:hover:text-red-400"
            >
              <LogOutIcon size={15} />
              <span>Sign Out</span>
            </button>
          ...
          {/* Stat Cards Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mt-8 pt-6 border-t border-slate-200/60 dark:border-slate-800/60">
            <div className="rounded-2xl border border-slate-100 bg-white/70 p-4 shadow-sm backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/50">
            ...
            <div className="rounded-2xl border border-slate-100 bg-white/70 p-4 shadow-sm backdrop-blur-sm dark:border-slate-800/60 dark:bg-slate-900/50">
              <div className="flex items-center gap-2 text-indigo-500 mb-1">
                <UsersIcon size={16} />
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Groups</span>
              </div>
              <p className="text-2xl font-black text-slate-900 dark:text-slate-100">
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-2 p-1.5 rounded-2xl bg-slate-200/60 dark:bg-slate-900/80 border border-slate-200/60 dark:border-slate-800/80 mb-6 overflow-x-auto">
          <button
          ...
          >
            <StarIcon size={15} filled={activeTab === "favorites"} />
            <span>Favorite Books ({favorites.length})</span>
          </button>
          ...
          >
            ...
            <span>Reading Progress ({inProgress.length})</span>
          </button>
        ...
        {loading ? (
          ...
          favorites.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white/50 p-12 text-center dark:border-slate-800 dark:bg-slate-900/50">
              <StarIcon size={36} className="text-slate-300 dark:text-slate-600 mb-3" />
              ...
              <Link
                href="/library"
                className="mt-5 px-5 py-2.5 rounded-2xl bg-brand-600 text-xs font-bold text-white shadow-md shadow-brand-500/20 hover:bg-brand-500 transition-all"
              >
                Explore Library
              </Link>
            </div>
          ...
          inProgress.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white/50 p-12 text-center dark:border-slate-800 dark:bg-slate-900/50">
              <ClockIcon size={36} className="text-slate-300 dark:text-slate-600 mb-3" />
              <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                No Reading History
              </h3>
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Start reading any book to track your reading progress.
              </p>
            </div>
          )
        ) : (
          groups.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white/50 p-12 text-center dark:border-slate-800 dark:bg-slate-900/50">
              <UsersIcon size={36} className="text-slate-300 dark:text-slate-600 mb-3" />
              <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
                No Groups Joined
              </h3>
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Create a reading group or join one with an invite code.
              </p>
              <Link
                href="/groups"
                className="mt-5 px-5 py-2.5 rounded-2xl bg-brand-600 text-xs font-bold text-white shadow-md shadow-brand-500/20 hover:bg-brand-500 transition-all"
